...
var GameStatus;
(function (GameStatus) {
    GameStatus[GameStatus["Prepare"] = 0] = "Prepare";
    GameStatus[GameStatus["Result"] = 1] = "Result";
...
var AIMode;
(function (AIMode) {
    AIMode[AIMode["Random"] = 0] = "Random";
    AIMode[AIMode["Mode2"] = 1] = "Mode2";
})(AIMode || (AIMode = {}));
...
function onClick(event) {
    // 顯示出牌的圖示
    const i = player1.lastCardId = player1.cards.indexOf(event.target);
    if (i < 0)
        return;
...
function handleAI() {
    ...
    //         maxIndex = i;
    // }
    const maxIndex = statics.reduce((p, v, i) => statics[p] < v ? i : p, RPS.Rock);
    switch (maxIndex) {
        case RPS.Rock:
